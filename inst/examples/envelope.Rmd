...
```{r}
p <- list()
for(i in c("crawler", "host", "gravid")) {
  env <- ewing_envelope(out, "host", i)
  p[[i]] <- ggplot_ewing_envelope(env)
}
```

```{r}
p[[1]] + p[[2]] + p[[3]] + plot_layout(nrow = 3)
```
...
```{r}
p <- list()
for(i in c("young", "adult")) {
  env <- ewing_envelope(out, "parasite", i)
  p[[i]] <- ggplot_ewing_envelope(env)
}
```

```{r}
p[[1]] + p[[2]] + plot_layout(nrow = 2)
```
...
for(i in c("crawler", "host", "gravid")) {
  env <- ewing_envelope(out, "host", i)
  res <- GET::fBoxplot(env, type = 'area', factor = 1.5, coverage = 0.5)
  p[[i]] <- plot(res) + ggplot2::labs(x = "time", y = i)
}
```
