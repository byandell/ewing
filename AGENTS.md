...
Key packages: `dplyr`, `tidyr`, `ggplot2`, `tibble`, `rlang`, `purrr`, `readxl`, `shiny`, `bslib`, `patchwork`, `GET`, `sf`, `splines`, `DT`. Optional GIS extension: `hexmap` (`byandell/hexmap`).
...
- **Standard Shinylive Pattern:** All Quarto Shinylive `.qmd` demos in `demos/` should use standard `shinylive-r` WebAssembly code blocks with `webr::install("byandell/ewing")` and `library(ewing)` for serverless package loading.
