                             ...
                             data )
{
  ...
  if( missing( data )) {
    if( !is.null( mvalue )) {
      mvalue <- predict( mvalue$meanvalue, mvalue$meanvalue$knots )
  ...
  tol = 1e-5, n = 1 )
{
  ...
  plotit <- function( xy, fig = "mean value", fit = interpSpline( xy$x, xy$y ))
  {
    switch( fig, {
        y <- xy$y
        ylim <- range(c(0,y))
      ...
      density = {
        y <- spline.rate( fit, xy$x )$y * exp( - predict( fit, xy$x )$y )
        tmp <- spline.rate( fit )
        tmp$y <- tmp$y * exp( - predict( fit )$y )
        ylim <- range(c(0,tmp$y))
      }
    )
    ...
    mtext( "time", 1, 2 )
    mtext( fig, 2, 2 )
    abline( v = max( xy$x ), lty = 2 )

    switch( fig, {
        if( fig == "probability" ) {
          tmp <- c(.1,.2,.5,1:10)
          ltmp <- 1-exp(-tmp)
          mtext( "mean value", 4, 2 )
        }
        else {  
          tmp <- c(seq(0,.9,,by=.1),.95,.98,.99,.999)
          ltmp <- -log(1-tmp)
          mtext( "probability", 4, 2 )
        }
        usr <- par("usr")
    ...
    data = FALSE )
  {
    ...
    if( any( tmp ))
      text( rep(usr[2],sum(tmp)), cmdlocs[tmp], cmds[tmp], col = rest, adj = 1 )
    if( any( !tmp ))
      text( rep(usr[2],sum(!tmp)), cmdlocs[!tmp], cmds[!tmp], col = col, adj = 1 )
  }
  ...
  tmp$fit <- NULL
  tmp$invmvalue <- backSpline( tmp$meanvalue )
...
{
  tmpar <- par( col = col )
  mtext( paste( "mean =", round( sums[1], 2 )), 3, at = par("usr")[1], adj = 0 )
  medianshow <- if( is.na( sums[2] ))
    "curve not monotone"
  else
    paste( "median =", round( sums[2], 2 ))
  mtext( medianshow, 3, at = par("usr")[2] / 1.25, adj = 1 )
  par( tmpar )
  invisible( sums )
}
###########################################################################################
curve.plot <- function( xy = seq(0,1,by=.25), y = c(0,.1,.5,.9,1),
  z = locator(1,"n"), n=5, action="add", fit = interpSpline( xy$x, xy$y ),
  ...
  f = function( x ) x, finv = function( x ) x )
{
  ...
  remove.points <- function( xy, finv, save.ends = TRUE ) {
    ...
    tmpd <- xy[tmp,"y"]
    tmpd <- finv( tmpd )
    points( xy[tmp,"x"], tmpd, lwd = lwd, col = "white" ) 
    tmp
  }
  for( i in 1:n) {
    switch( action,
    ...
    tmpp <- predict( fit )
    lines( tmpp$x, finv( tmpp$y ), col = col[1], lwd = lwd )

    adiff <- diff( coef( fit )[,1] )
    if( backfit & ( all( adiff < 0) || all( adiff > 0 ))) {
      ## backspline (not quite a spline) fit to inverse
      tmpback <- predict( backSpline( fit ))
      lines( tmpback$y, finv( tmpback$x ), col = col[2], lwd = lwd )
    }
  }
  ...
  col = c("green","blue","red","orange") )
{
  ...
  for( i in seq( length( conf ))) {
    ## lower confidence
    tmp <- qbinom( conf[i], n, prob, lower.tail = TRUE ) / ( n + 1 )
    ...
    if( rate )
      tmp <- - log( 1 - tmp )
    lines( data, tmp, lty = 2, col = col[i] )
    ## upper confidence
    tmp <- qbinom( conf[i], n, prob, lower.tail = FALSE ) / ( n + 1 )
    tmp <- 1 - exp( rescale * log( 1 - tmp ))
    tmpna <- is.na( tmp )
    if( any( tmpna ))
      tmp[tmpna] <- f[n]
...
spline.extrapolate <- function( meanvalue, invmvalue, x )
{
  ...
  coeff <- coef( invmvalue )
  nr <- nrow( coeff ) - 1
  xknot <- splineKnots( invmvalue )[nr+(0:1)]
  yknot <- splineKnots( meanvalue )[nr+1]
  ...
  u = seq( 0.01, 0.99, by = 0.01 ))
{
  ...
  for( it in truncation ) for( ii in intensity ) {
    tmpp <- predict(fit$invmvalue, x = ( G(it)+G(u))/ii)
    nay <- is.na( tmpp$y )
    if( any ( nay ))
  ...
  tol = 1e-5, legend.flag = 1, cex = 0.5, ylim = ylims, prefix = "" )
{
  ...
  mtext( main, 3, 1 )
  lines( ref[,1], ref[,2], lty = 3, lwd = 1 )
  abline( h = refmean * c( 1, goal[1] ), col = c("black","blue"), lty = c(1,3) )
  col <- c("blue","red","green","aquamarine","black")
  lty <- c(2,4,5,6,1)
  names( col ) <- names(lty) <- fives
  for( i in names( five.lines ))
    lines( tmp[,1], five.lines[[i]], lty = lty[i], lwd = 1, col = col[i] )
  switch( 1 + legend.flag,
    legend( 0, ylim[2], names( five.lines ),
      lty = lty[ names( five.lines ) ],
      col = col[ names( five.lines ) ], cex = cex ),
    legend( 1, ylim[1], names( five.lines ), xjust = 1, yjust = 0,
      lty = lty[ names( five.lines ) ],
      col = col[ names( five.lines ) ], cex = cex ))
...
five.plot <- function( gencurve = spline.meanvalue(), fit = gencurve$fit, pick, vals, ylim = ylims )
{
  ...
  require( splines )
  tmp <- predict( fit$invmvalue, x = G(tmpx))
  ylims <- range( tmp$y, na.rm = TRUE )
  tmp <- five.switch( fit, pick, vals )
  plot( 0:1, ylim, type = "n",
    xlab = "probability", ylab = "time" )
  title( main = pick )
  lines( tmp[,1], tmp[,2], lty = 3 )
  for( i in 3:ncol( tmp ))
    lines( tmp[,1], tmp[,i], lty = 1 )
}
###########################################################################################
        ...
        u = seq(0.01,.99,by=.01))
{
        ...
        tmp0 <- predict(invmvalue$fit$inv,x=G(u))
        tmpp <- predict(invmvalue$fit$inv,x=(G(truncation)+G(u))/intensity)
        tmpp$y[ u >= rejection ] <- NA
        ylims <- dispersion * range( tmp0$y, tmpp$y, na.rm = TRUE ) + location
        plot( c(0,1), ylim,
                type = "n",
                ylab = "time", xlab = "probability" )
        lines( u, dispersion * tmp0$y + location, lty = 3 )
        lines( u, dispersion * tmpp$y + location, lty = 1 )
}
###########################################################################################
	...
	u = seq(0.01,.99,by=.01), lty = 1 )
{
	...
	for( it in truncation ) for( ii in intensity )
	{
		tmpp <- predict(invmvalue$fit$inv, x = ( G(it)+G(u))/ii)
		for( ir in rejection )
		{
			yr <- tmpp$y
			yr[ u >= rejection ] <- NA
    ...
    y = y), n = 1, horizontal = FALSE) 
{
  ...
  ## plot curve and surrounding axes
  par( mfrow = c(1,1), mar = rep(4.1,4))
  plotit <- function( xy, fig = "temp", fit = interpSpline( xy$x, xy$y ),
  ...
  repeat {
    ...
    if(( !horizontal & z$x > separator ) | ( horizontal * z$y > separator )) {
      ...
      if( use.data ) {
        rx <- range( xy$x )
        dx <- range( datax )        
        lines( rx[1] + ( datax - dx[1] ) * diff( rx ) / diff( dx ), data * rescale.data )
      }
      plotcmd( ans, fig, cmds, cmdlocs, usr )
    }
    else {
