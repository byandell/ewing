---
title: "Substrate Geometry"
author: "Brian S. Yandell"
date: "`r format(Sys.time(), '%d %B %Y')`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Substrate Geometry}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

This document details the tridiagonal triangular coordinate system $(a, b, c)$, substrate network topology, unit coordinate rescaling ($[0, W_{sub}]$), discrete 6-sided hexagonal polygon tile generation, and spatial organism movement visualization modules.
...
- **Substrate Patch Resolution ([ewing_substrate](../../R/ewing_substrate.R#L44))**: Maps organism substrate stage indices `individual["sub.stage"]` to specific plant substrate elements (`fr1`, `fr2`, `fr3`, `fr4`, `twig`/`tw1`, `lftop`, `lfbot`).
- **Hexagonal Overlay Generation ([create_hex_overlay](../../R/substrate_triangle.R#L175))**: Constructs 6-vertex polygon tiles for each lattice dot in a `substrate` object, rendering discrete hexagonal cells across all substrate components (`fr1..fr4`, `tw1..tw2`, `lftop..lfbot`).
- **Per-Substrate Surface Coordinate Rescaling ([ewing_substrate](../../R/ewing_substrate.R#L60))**: Rescales organism local coordinates $(pos.a, pos.b, pos.c)$ into the unit triangle $[0, W_{sub}]$ of each active substrate patch (supporting custom substrate sizes $W_{sub}$ per surface). Applies topological offsets (`offset`) and orientations (`dir = "up"` or `"down"`), converting to global Euclidean $(x, y)$ coordinates via `tri2car()` so all organisms are visualized strictly within their designated substrate surface bounds.
- **Integrated Module Controls ([substrateApp](../../R/substrateApp.R#L35))**: Supports switching between `"hex"` (global hexagonal grid overlay) and `"facet"` (panel view faceted by substrate element), alongside action buttons for stepping through simulation events (`+1`, `+10`, `+100` steps).
- **Interactive Application ([hexmoveApp](../../R/hexmoveApp.R#L16))**: Composes `initParInput`, `initServer`, and `substrateServer` into an interactive exploration tool. See [hexmove.md](hexmove.md) for full documentation.
...
The `hexmoveApp` application visualizes host and parasite spatial positions mapped directly onto a unified hexagonal substrate network topology ([create_substrate](../../R/substrate_triangle.R#L66)).
...
[substrate_topology](../../R/substrate_triangle.R#L37) defines topological offsets $\mathbf{o} = (o_a, o_b, o_c)$ and component orientations (`dir = "up"` or `"down"`) for each substrate element. Substrate component names like `twig` are mapped to `tw1` to ensure every individual is placed precisely on its active substrate patch.
...
A discrete hexagonal grid overlay is generated over the substrate lattice points (`sub_obj$points`) using [create_hex_overlay()](../../R/substrate_triangle.R#L175).
...
The interactive controls in [substrateInput](../../R/substrateApp.R#L35) and [substrateServer](../../R/substrateApp.R#L80) provide:
...
The app launcher script is located at [inst/scripts/hexmoveApp.R](../../inst/scripts/hexmoveApp.R).
